...
def studentApi(request,id=0):
    ...
    if request.method=='GET':
        ...
        if datetime.datetime.strptime(student_data[DOB], '%Y-%m-%d') >= today:
            try:
                getattr(models, STUDENT).objects.get(**{EMAIL: student_data[EMAIL]})
            ...
            return JsonResponse("Date of birth needs to be greater than today",safe=False)
    elif request.method=='PUT':
        student_data=JSONParser().parse(request)
        student=getattr(models, STUDENT).objects.get(StudentId=student_data['id'])
        if datetime.datetime.strptime(student_data[DOB], '%Y-%m-%d') >= today:
            try:
                getattr(models, STUDENT).objects.get(**{EMAIL: student_data[EMAIL]}).exclude(**{PK: id})
                return JsonResponse("Failed to Update as email adress aleady exist",safe=False)
            except Exception as ex:
